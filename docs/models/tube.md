...
`Drive ≈ 0` is transparent via a short dry/wet blend (no hard cliff into a near-silent stage).
...
| Setting | Intent |
|---------|--------|
| Drive **0** | Transparent (short dry/wet → identity) |
| Drive **~0.25+** | Audible / continuous (no gated dead zone) |
...
Drive→`Vdrive` map (`kDriveCurve≈1.6`) + makeup remain **plugin control / loudness** scaffolding (see Tech debt).

## Tech debt / teaching approximations

Track these so we do not pretend the stage is a full datasheet model:

- **Drive→`Vdrive` map** (`kMaxGridGain` / `kDriveCurve≈1.6`) is phenomenological, not a pot/network. `|Vdrive|` is soft-limited (`tanh` to ~18 V) so Drive 1 stays Newton-friendly under 4× OS — not a substitute for input coupling-cap topology. A short dry/wet blend near Drive 0 avoids a silent dead zone.
