...
**Diode aliasing under Plugin Doctor sweeps** still shows clear Nyquist foldback at default Drive (Newton path, OS-only, no ADAA). Raising OS factor and/or diode-side AA is parked while Tube flavors advance — re-open with `diode_aliasing_verify` + Plugin Doctor when returning to diode.
